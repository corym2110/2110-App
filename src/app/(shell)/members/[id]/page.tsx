...
import { MEMBERS, memberById, initialsOf } from "@/data/mock/members";
...
export default function MemberProfilePage() {
  ...
  const addShared = useSharedAccountsStore((s) => s.add);
  const removeShared = useSharedAccountsStore((s) => s.remove);
  ...
  const paysFor = byPayer[member.name] ?? [];
  const payerNames = Object.keys(byPayer).filter((p) => byPayer[p].includes(member.name));
  const candidates = MEMBERS.filter((m) => m.name !== member.name && !paysFor.includes(m.name));
          ...
          {addOpen && (
            <div className="mb-3.5 flex gap-1.5">
              ...
              <button
                ...
                onClick={() => {
                  if (!addPick) return;
                  addShared(member.name, addPick);
                  setAddOpen(false);
                  setAddPick("");
                }}
                className="h-[34px] flex-none rounded-lg bg-accent px-3.5 text-[13px] font-semibold text-on-accent"
              >
                Link
              </button>
            </div>
              ...
              <div className="mb-3.5 flex flex-col gap-1">
                {paysFor.map((n) => (
                  <div key={n} className="-mx-2 flex items-center gap-1">
                    <Link href={`/members/${MEMBERS.find((m) => m.name === n)?.id ?? ""}`} className="flex min-w-0 flex-1 items-center gap-2.5 rounded-lg px-2 py-1.5 text-[13.5px] text-fg hover:bg-row">
                    ...
                    <button
                      type="button"
                      onClick={() => removeShared(member.name, n)}
                      title="Remove link"
          ...
          {payerNames.length > 0 && (
            <>
              ...
              <div className="flex flex-col gap-1">
                {payerNames.map((n) => (
                  <Link key={n} href={`/members/${MEMBERS.find((m) => m.name === n)?.id ?? ""}`} className="-mx-2 flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-[13.5px] text-fg hover:bg-row">
                    <span className="grid h-6 w-6 flex-none place-items-center rounded-full bg-row text-[10px] font-semibold text-muted">
                      {initialsOf(n)}
                    </span>
                    <span className="min-w-0 truncate">{n}</span>
        ...
        {history.map((h) => {
          ...
          return (
            <div key={h.date} className="flex items-center gap-4 border-b border-divider py-2.5 last:border-b-0">
              ...
              <span className={`w-24 flex-none rounded-md py-0.5 text-center text-[11.5px] ${paid ? "bg-ok/15 text-ok" : "bg-bad/10 text-bad"}`}>
                {h.pay}
              </span>
            </div>
          );
